...
void PrintDie(unsigned int dieNo ) {
    xil_printf("Printing Die: %d\r\n", dieNo);
    xil_printf("head: %d, tail, %d\r\n", bufLruList->bufLruEntry[dieNo].head, bufLruList->bufLruEntry[dieNo].tail);

    unsigned int bufferEntry;
    for(bufferEntry = bufLruList->bufLruEntry[dieNo].head;
            bufferEntry != 0x7fff;
            bufferEntry = bufMap->bufEntry[bufferEntry].nextEntry) {
      xil_printf("buffEntry: %d, lpn: %d, dirty: %d, nextEntry: %d, prevEntry: %d\r\n",
             bufferEntry,
             bufMap->bufEntry[bufferEntry].lpn,
             bufMap->bufEntry[bufferEntry].dirty,
             bufMap->bufEntry[bufferEntry].nextEntry,
             bufMap->bufEntry[bufferEntry].prevEntry);
    }
}
