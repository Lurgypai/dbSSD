...
#define JOIN_PAGE_ADDR (BUFFER_ADDR + (BUF_ENTRY_NUM) * BUF_ENTRY_SIZE) // THIS HAS TO BE HERE?

#define SPARE_ADDR			(JOIN_PAGE_ADDR + BUF_ENTRY_SIZE)
...
#define BUFFER_LRU_LIST_ADDR 	(BUFFER_MAP_ADDR + sizeof(struct bufEntry) * (BUF_ENTRY_NUM + 1)) // add an extra entry at the end for us to use
...
//hash map
#define HASHMAP_ADDR 0x30000000
#define TABLE_ARR_ADDR (HASHMAP_ADDR + MAX_BACKING_SIZE)
